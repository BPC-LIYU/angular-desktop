var service_app = angular.module('desktop.services', ['ngCookies'])
   ...
   * 处理接口状态码
   by: 范俊伟 at:2016-02-18
   */
  .service("globalStateCheck", function ($ionicHistory, $state, $injector) {
   ...
   * by: 闫宇 at:2016-02-25
   */
  .factory('Auth', function ($q, $injector, $state, $ionicHistory, $rootScope, KeyValueStore) {
    var httpReq = $injector.get('httpReq');
    var Store = $injector.get('Store');
    var myUserInfo = $injector.get('myUserInfo');
    var myProjectList = $injector.get('myProjectList');
    return {
      has_login: function () {
        return Store.get("has_login");
      },
      login: function (username, password) {
        ...
        httpReq("/ns/user/simple_login", {
          ...
          if (data.success) {
            Store.set("ttjd_sessionid", data.result.sessionid);
            Store.set("has_login", true);
            KeyValueStore.set("last_login_tel", username);
            deferred.resolve();
          }
          else {
            deferred.reject();
      ...
      check_login: function () {
        ...
        httpReq("/ns/user/check_login").then(function (data) {
          var has_login = data.result.has_login;
          if (!has_login) {
            Store.set("has_login", false);
          }
          deferred.resolve(has_login);
        }, function () {
          deferred.reject();
        });
        return deferred.promise;
      ...
      logout: function () {
        ...
        try {
          hx.logout();
          $rootScope.hx_has_login = false;
        }
        catch (e) {
        }
        httpReq("/ns/user/logout").then(function () {
          Store.set("has_login", false);
  ...
  .factory('AppList', function ($q, httpReq, $injector,$rootScope) {
    var app_map = null;
    var last_project_id = null;
    var app_list = null;
    var myUserInfo = $injector.get("myUserInfo");

    function getStoreKey(app, key) {
      var uid = myUserInfo.getUid();
      return uid + "_" + app.id + "_" + key;
    }

    function get_unread_num(project_id, app_list) {

      httpReq("/ns/project/get_unread_num_by_project_id", {project_id: project_id}).then(function (data) {
        $rootScope.$broadcast("reload_applicaton");
        _(data.result).each(function (unread_item) {
          var app = _(app_list).find(function (item) {
            return item.flag == unread_item.flag;
          });
          if (app) {
            app.unreadnum = unread_item.unreadnum;
          }
        });
      });
    }

    function getMap(project_id) {
      var deferred = $q.defer();
      if (app_map && project_id == last_project_id) {
        get_unread_num(project_id, app_list);
        deferred.resolve(app_map);
      }
      if (!myUserInfo.getUid()) {
        myUserInfo.getUserInfo();
      }

      httpReq("/ns/project/query_app_list", {project_id: project_id}).then(function (data) {
        last_project_id = project_id;
        app_list = data.result;
        _(app_list).each(function (item) {
          item.getStoreKey = function (key) {
            return getStoreKey(item, key);
          }
        });
        app_map = _(app_list).groupBy("father");
        get_unread_num(project_id, app_list);
        deferred.resolve(app_map);
      });
    ...
    return {
      ...
      init: function (project_id) {
        app_map = null;
        return getMap(project_id);
      },
      get_app_by_id: function (project_id, id) {
        var deferred = $q.defer();
        getMap(project_id).then(function () {
          var app = _(app_list).find(function (item) {
            return item.id == id;
          });
  ...
  .factory('myProjectList', function ($q, httpReq) {
    var my_project = null;
    return {
      clear: function () {
        my_project = null;
      ...
      get: function () {
        ...
        if (my_project) {
          has_return = true;
          deferred.resolve(my_project);
        }
        httpReq("/ns/project/query_my_project", {field_name: "name"}).then(function (data) {
          my_project = data.result;
          if (!has_return) {
            deferred.resolve(my_project);
          }
        });
